...
if _plat=='Linux':
  _lib = ctypes.CDLL("./include/libtoolkit.so")
elif _plat=='Windows':
  try:
...
def ENgetpumpswitches(index):
	"""Retrieves the switches of a pump with a specified index.
	Arguments:
	index: node index"""
	j= ctypes.c_int()
	ierr= _lib.ENgetpumpswitches(index, ctypes.byref(j))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j.value

def ENgetminstoptime(index):
	"""Retrieves the min stop time of a pump with a specified index.
	Arguments:
	index: node index"""
	j= ctypes.c_int()
	ierr= _lib.ENgetminstoptime(index, ctypes.byref(j))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j.value
...
def ENgetpumpindex(index):
	"""Retrieves the indexes of the nodes of a specified pump.
    Arguments:
    index: pump index"""
	
	j1= ctypes.c_int()
	ierr= _lib.ENgetpumpindex(index, ctypes.byref(j1))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j1.value

def ENgettankindex(index):
	"""Retrieves the indexes of the nodes of a specified tank.
    Arguments:
    index: pump index"""
	
	j1= ctypes.c_int()
	ierr= _lib.ENgettankindex(index, ctypes.byref(j1))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j1.value
...
def ENgettotaldemand( ):
	j=ctypes.c_float()
	ierr= _lib.ENgettotaldemand(ctypes.byref(j))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j.value

def ENgettotalinflow( ):
	j=ctypes.c_float()
	ierr= _lib.ENgettotalinflow(ctypes.byref(j))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j.value

def ENgettotalleakage( ):
	j=ctypes.c_float()
	ierr= _lib.ENgettotalleakage(ctypes.byref(j))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j.value

def ENgettotalenergycost( ):
	j=ctypes.c_float()
	ierr= _lib.ENgettotalenergycost(ctypes.byref(j))
	if ierr!=0: raise ENtoolkitError(ierr)
	return j.value
